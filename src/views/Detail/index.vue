          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSale, index) in spuSaleAttrList" :key="index">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: spuSaleList.isChecked == 1 }"
                  v-for="(spuSaleList, index) in spuSale.spuSaleAttrValueList"
                  :key="spuSaleList.id"
                  @click="
                    changeChecked(spuSaleList, spuSale.spuSaleAttrValueList)
                  "
                >
                  {{ spuSaleList.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  @change="changeStuNum"
                />
                <a href="javascript:" class="plus" @click="stuNum++">+</a>
                <a
                  href="javascript:"
                  class="mins"
                  @click="stuNum > 1 ? stuNum-- : (stuNum = 1)"
                  >-</a
                >
              </div>
              <div class="add">
...
export default {
  ...
  mounted() {
    this.$store.dispatch("getDetailList", this.$route.params.stuId);
  },
  computed: {
    ...mapGetters(["categoryView", "skuInfo", "spuSaleAttrList"]),
  },
  methods: {
    changeChecked(spuSaleList, arr) {
      arr.forEach((element) => {
        element.isChecked = 0;
      });
      spuSaleList.isChecked = 1;
    },
    changeStuNum() {
      if (this.stuNum.length > 2) {
        this.stuNum = 1;
      } else {
        this.stuNum = parseInt(this.stuNum);
      }
    },
    async addCart() {
      try {
