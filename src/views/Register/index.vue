<template>
  <div class="register-container">
    <!-- 注册内容 -->
      ...
      <div class="content">
        <label>验证码:</label>
        <input type="text" placeholder="请输入验证码" :value="getCode">
        <button @click="getVerificationCode" style="padding: 9px; margin-right: -80px" v-if="getCode == null">获取验证码</button>
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="content">
        <label>登录密码:</label>
      ...
      <div class="btn">
        <button @click="getRegistrationIsCompleted">完成注册</button>
      </div>
    </div>
    <!-- 底部 -->
    <div class="copyright">
...
<script>
  ...
  export default {
    ...
    methods: {
      getVerificationCode() {
        if(this.phone !== "") {
          this.$store.dispatch("reqVerificationCode",{phone: this.phone})
        }else {
          alert('手机号不能为空')
        }
      },
      getRegistrationIsCompleted() {
       if(this.agree && this.password === this.password1 ){
